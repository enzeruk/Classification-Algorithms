...
# Classification algorithm 2a: Neural Network with 1 hidden layer & Gradient Descent (GD) #
# train_set & test_set, that are passed, keep the Y category info
def nn1_gd_algorithm(train_set, test_set, k):
    nn1_gd_metrics = []
    ...
    mlp = MLPClassifier(activation='logistic', solver='sgd', hidden_layer_sizes=(k, ), batch_size=train_size,
                        max_iter=500)       # epochs = 500 (max_iter)
    ...
    nn1_gd_metrics.append(accuracy)
    nn1_gd_metrics.append(f1score)

    return nn1_gd_metrics
